...
void gb_cartridge_load_ram(gb_t* gb,const char* path){
    ...
    if(len != cartridge->ram_length) goto end;

    fread(cartridge->ram,1,cartridge->ram_length,file);
...
size_t gb_cartridge_get_rom_length(uint8_t* header){
    ...
    size_t rom_length = 0x00;

    switch(header[0x48]){
        //32KB
        case 0x00: rom_length = 0x8000; break;
        //64KB
        case 0x01: rom_length = 0x10000; break;
        //128KB
        case 0x02: rom_length = 0x20000; break;
        //256KB
        case 0x03: rom_length = 0x40000; break;
        //512KB
        case 0x04: rom_length = 0x80000; break;
        //1MB
        case 0x05: rom_length = 0x100000; break;
        //2MB
        case 0x06: rom_length = 0x200000; break;
        //4MB 
        case 0x07: rom_length = 0x400000; break;
        //8MB 
        case 0x08: rom_length = 0x800000; break;
    }
...
bool gb_cartridge_init_ram(gb_cartridge_t* cartridge,bool battery){
    ...
    switch(cartridge->header[0x49]){
        //2KB
        case 0x01:
            cartridge->ram_length = 0x800;
            cartridge->ram_bank_mask = 0x00;
            break;
        //8KB
        case 0x02:
            cartridge->ram_length = 0x2000;
            cartridge->ram_bank_mask = 0x00;
            break;
        //32KB
        case 0x03:
            cartridge->ram_length = 0x8000;
            cartridge->ram_bank_mask = 0x03;
            break;
        //128KB
        case 0x04:
            cartridge->ram_length = 0x20000;
            cartridge->ram_bank_mask = 0x0F;
            break;
        //64KB
        case 0x05:
            cartridge->ram_length = 0x10000;
    ...
    if(cartridge->ram_length > 0x00){
        ...
        cartridge->ram = (uint8_t*)malloc(cartridge->ram_length);
        ...
        memset(cartridge->ram,0x00,cartridge->ram_length);
...
size_t gb_cartridge_rom_absolute_address(gb_cartridge_t* cartridge,uint16_t relative_address){
    if(cartridge->mapper.rom_absolute_address != NULL){
        return cartridge->mapper.rom_absolute_address(cartridge,relative_address);
    }
    else{
        return relative_address & 0x7FFF;
    }
}

size_t gb_cartridge_ram_absolute_address(gb_cartridge_t* cartridge,uint16_t relative_address){
    if(cartridge->mapper.ram_absolute_address != NULL){
        return cartridge->mapper.ram_absolute_address(cartridge,relative_address);
    }
    else{
        return relative_address & 0x7FFF;
    }
